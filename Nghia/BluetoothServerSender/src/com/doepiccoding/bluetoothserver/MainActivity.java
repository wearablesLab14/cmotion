...
//		 Always make sure that Bluetooth server is discoverable during
//		 listening...
...
//		 DISCOVERABLE_REQUEST_CODE);
	 ...
	 * resultCode, data); android.util.Log.e("TrackingFlow",
	 * "Creating thread to start listening..."); new Thread(reader).start(); }
	 */
			...
			try {
				...
				while (CONTINUE_READ_WRITE) {
					...
					runOnUiThread(new Runnable() {
						@Override
						public void run() {
							// todo can show the coordinate or update
							// lastCoordinate
		...
		public void run() {
			...
			while (CONTINUE_READ_WRITE) {
				try {
					...
					os.write("Sending some Dummy Coord " + Math.random() + "\n");
					os.flush();
					Thread.sleep(100);
				} catch (Exception e) {
					e.printStackTrace();
